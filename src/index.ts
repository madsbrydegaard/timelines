...
export const TimelineContainer = (
  ...
  const registerListeners = (element: HTMLElement): void => {
    ...
    const onEventSelected = (
      event: CustomEvent<ITimelineCustomEventDetails>,
    ) => {
  ...
  const createEventHTML = (
    ...
    for (const timelineEvent of parentEvent.timelineEventDetails
      ...
      timelineEvent.timelineEventDetails.eventNode.style.left =
        leftRatio * 100 + "%";
      timelineEvent.timelineEventDetails.eventNode.style.width =
        widthRatio + "%";
      timelineEvent.timelineEventDetails.eventNode.attributes["starttime"] =
        viewInside
          ? viewStart()
          : timelineEvent.timelineEventDetails.startMinutes;
      eventsFragment.append(timelineEvent.timelineEventDetails.eventNode);
  ...
  const addEvents = (
    ...
    parent.timelineEventDetails.height = Math.max(
      ...Object.entries(parent.timelineEventDetails.timelineLevelMatrix).map(
        ([key, o]) => Number(key),
      ),
    );
  };
  const parseEvent = (
    ...
    timelineEventWithDetails.timelineEventDetails.startMinutes = calcStart(
