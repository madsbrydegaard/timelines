...
export interface ITimelineContainer {
  ...
  zoom: (
    ...
    onzoomend?: (timelineEvent: ITimelineEvent) => void,
  ) => void;
  focus: (
    timelineEvent: ITimelineEvent,
    useAnimation?: boolean,
    onfocused?: (timelineEvent: ITimelineEvent) => void,
  ) => void;
  reset: () => void;
...
type TimelineResolvedOptions = Omit<
  Required<ITimelineOptions>,
  "classNames"
> & {
  classNames: Required<NonNullable<ITimelineOptions["classNames"]>>;
};

export const TimelineContainer = (
  elementIdentifier: HTMLElement | string,
  settings?: ITimelineOptions,
): ITimelineContainer => {
  let ratio: number;
  let pivot: number;
  ...
  let options: TimelineResolvedOptions;
  ...
  const isITimelineEventWithDetails = (
    timelineEvent: ITimelineEvent,
  ): timelineEvent is ITimelineEventWithDetails =>
    "timelineEventDetails" in timelineEvent;
  const add = (...timelineEvents: ITimelineEvent[]): void => {
    if (!timelineEvents)
      throw new Error(
        `Event argument is empty. Please provide Timeline event(s) as input`,
      );
  ...
  const isViewInside = (timelineEvent: ITimelineEventWithDetails): boolean => {
    return (
      timelineEvent.timelineEventDetails.startMinutes < viewStart() &&
      timelineEvent.timelineEventDetails.endMinutes > viewEnd()
    );
  };
  const init = (
    elementIdentifier: HTMLElement | string,
    ...
    // Handle options
    const defaults: TimelineResolvedOptions = {
      labelCount: 5,
      zoomSpeed: 0.04,
      dragSpeed: 0.001,
      timelineStart: "-15B",
    ...
    const parsedRootTimeline = parseEvent({
      title: "View",
      type: "container",
      start: options.start,
      end: options.end,
    });
    if (!parsedRootTimeline) {
      throw new Error("Failed to initialize root timeline");
    }
    rootTimeline = parsedRootTimeline;

    // Set timeline boundaries
    timelineStart =
      parseDateToMinutes(options.timelineStart) ??
      rootTimeline.timelineEventDetails.startMinutes;
    timelineEnd =
      parseDateToMinutes(options.timelineEnd) ??
      rootTimeline.timelineEventDetails.endMinutes;
  ...
  const findFirstEvent = (
    timelineEventIdentifier: string,
    parent?: ITimelineEventWithDetails,
  ): ITimelineEventWithDetails | undefined => {
    let result: ITimelineEventWithDetails | undefined = undefined;
    ...
    for (const child of parentNode.timelineEventDetails.childrenByStartMinute) {
      if (
        child.title === timelineEventIdentifier ||
        child.timelineEventDetails.id === timelineEventIdentifier
      ) {
        result = child;
        break;
      } else {
  ...
  const zoom = (
    ...
    zoomto(
      startMinutes ?? timelineEvent.timelineEventDetails.startMinutes,
      endMinutes ?? timelineEvent.timelineEventDetails.endMinutes,
      useAnimation,
      () => {
  ...
  const calcPivotRatio = (
    startMinutes: number,
    endMinutes: number,
  ): { pivot: number; ratio: number } => {
  ...
  const zoomto = (
    ...
    const { pivot: targetPivot, ratio: targetRatio } = calcPivotRatio(
      startMinutes,
      endMinutes,
    );
  ...
  const registerListeners = (element: HTMLElement): void => {
    ...
    const hover = (clientX: number, clientY: number) => {
      ...
      const timelineEvent = visibleEvents.find(
        (ev) =>
          ev.timelineEventDetails.id === hoverEvent.getAttribute("eventid"),
      );
      if (timelineEvent) {
    ...
    const onEventClick = (event: Event) => {
      const customEvent = event as CustomEvent<ITimelineCustomEventDetails>;
      if (options.autoSelect && customEvent.detail.timelineEvent) {
        select(customEvent.detail.timelineEvent.timelineEventDetails.id);
      }
    };
    const onEventSelected = (event: Event) => {
      const customEvent = event as CustomEvent<ITimelineCustomEventDetails>;
      if (options.autoZoom && customEvent.detail.timelineEvent) {
        zoom(customEvent.detail.timelineEvent);
      }
    };
    const click = (clientX: number, clientY: number) => {
      ...
      if (!clickedEvent) {
        if (
          options.autoDeselectOutside &&
          !preventPreviewRender &&
      ...
      const timelineEvent = visibleEvents.find(
        (ev) => ev.timelineEventDetails.id === eventid,
      );
      if (timelineEvent) {
    ...
    element.addEventListener(
      ...
      (event: TouchEvent) => {
        ...
        if (
          ...
          // the 2-touch
          const touch1 = tpCache.findIndex(
            (tp) => tp.identifier === event.targetTouches[0].identifier,
          );
          const touch2 = tpCache.findIndex(
            (tp) => tp.identifier === event.targetTouches[1].identifier,
          );
          ...
          if (touch1 >= 0 && touch2 >= 0) {
            ...
            const diff1 = Math.abs(
              tpCache[touch1].clientX - tpCache[touch2].clientX,
            );
        ...
        if (
          ...
          if (touch1 >= 0) {
            // Calculate the difference between the start and move coordinates
            const diffX =
              event.targetTouches[0].clientX - tpCache[touch1].clientX;
            if (diffX !== 0) {
              inDrag = true;
              touchDragFactor = 3;
    ...
    element.addEventListener("click.tl.event", onEventClick as EventListener);
    element.addEventListener(
      "selected.tl.event",
      onEventSelected as EventListener,
    );
  ...
  const createPreviewHTML = (): DocumentFragment | undefined => {
    ...
    const highscores = visibleEvents
      ...
      .filter(
        (
          evt,
        ): evt is ITimelineEventWithDetails & {
          timelineEventDetails: ITimelineEventDetails & {
            previewNode: HTMLDivElement;
            eventNode: HTMLDivElement;
          };
        } =>
          !!evt.timelineEventDetails.previewNode &&
          !!evt.timelineEventDetails.eventNode,
      )
      ...
      .sort(
        (a, b) =>
          a.timelineEventDetails.startMinutes -
    ...
    for (const [i, timelineEvent] of highscores.entries()) {
      ...
      const randomLeftPosition =
        fraction * i + fraction / 2 - previewWidthFactor / 2;
      const randomTopPosition = Math.random() / 3 + 0.08;

      const createTimelinePreviewHTML = (): void => {
        timelineEvent.timelineEventDetails.previewNode.style.left =
          randomLeftPosition * 100 + "%";
        timelineEvent.timelineEventDetails.previewNode.style.top =
          randomTopPosition * 100 + "%";

        let x2 = getViewRatio(
          timelineEvent.timelineEventDetails.startMinutes +
            timelineEvent.timelineEventDetails.durationMinutes / 2,
        );
        ...
        if (x2 > 1) {
          x2 = getViewRatio(
            timelineEvent.timelineEventDetails.startMinutes +
              (viewEnd() - timelineEvent.timelineEventDetails.startMinutes) / 2,
          );
        }
        if (x2 < 0) {
          x2 = getViewRatio(
  ...
  const createEventHTML = (
    ...
    for (const timelineEvent of parentEvent.timelineEventDetails
      ...
      const leftRatio = viewInside
        ? 0
        : getViewRatio(timelineEvent.timelineEventDetails.startMinutes);
      const widthRatio = viewInside
        ? 100
      ...
      const isHighlighted =
        ...
        !!selectedTimelineIds.find(
          (tlId) => tlId === timelineEvent.timelineEventDetails.id,
        );
      const levelFactor =
        (timelineEvent.timelineEventDetails.level - 1) * options.eventHeight +
        timelineEvent.timelineEventDetails.level * options.eventSpacing;
      const stepFactor =
        (parentEvent.timelineEventDetails.step - 1) * options.eventSpacing;
      const eventNode = timelineEvent.timelineEventDetails.eventNode;
      if (!eventNode) {
        continue;
      }
      ...
      switch (timelineEvent.type) {
        default: {
          const nested = createEventHTML(timelineEvent);
          if (nested) eventsFragment.append(nested);
          continue;
        }
        case "container": {
          const heightFactor =
          ...
          eventNode.style.bottom = `${heightFactor + levelFactor + stepFactor}px`;
          eventsFragment.append(eventNode);
          const nested = createEventHTML(timelineEvent);
          if (nested) eventsFragment.append(nested);
          continue;
        }
        case "timeline": {
          ...
          eventNode.style.bottom = `${parentfactor + levelFactor + stepFactor}px`;
          eventNode.style.backgroundColor = isHighlighted
      ...
      eventNode.style.left = leftRatio * 100 + "%";
      eventNode.style.width = widthRatio + "%";
  ...
  const appendLabelHTML = (): void => {
    ...
    const integerDifFraction = Math.floor(
      timelineViewDifference / currentTimestampDistanceByLevel,
    );
    ...
    for (let i = 0; i < options.labelCount + 2; i++) {
      const labelTime =
        (i + 1) * currentTimestampDistanceByLevel +
        timelineStart +
        currentDif -
        currentTimestampDistanceByLevel;
  ...
  const formatDateLabel = (minutes: number): string => {
    ...
    const momentInValidateRange =
      ...
      minutes < 270000 * MINUTES_IN_YEAR;
    const date = momentInValidateRange
      ? new Date(minutes * 6e4)
      : new Date(currentRemainder * 6e4);
  ...
  const clear = (): void => {
    ...
    currentTimeline.timelineEventDetails.timelineLevelMatrix = {
    ...
    const parseDateString = (input: string): number => {
      switch (input) {
        ...
        default:
          ...
          // 525948.766 = minutes in a year
          const years = input.match(/y$/)
            ? Number(input.replace(/y$/, ""))
            : NaN;
          if (!isNaN(years)) {
  ...
  const calcEnd = (
    ...
    const result = timelineEventWithDetails.timelineEventDetails
      ...
      ? Math.max.apply(
          1,
          timelineEventWithDetails.timelineEventDetails.childrenByStartMinute.map(
            (child) => child.timelineEventDetails.endMinutes,
          ),
        )
    ...
    const result = timelineChildren.length
      ? Math.min(
          timelineEventWithDetails.timelineEventDetails
            .startMinutesForTimelineChildren ?? Number.MAX_SAFE_INTEGER,
    ...
    const result = timelineChildren.length
      ? Math.max(
          ...timelineChildren
            .map(
              (child) =>
                child.timelineEventDetails.endMinutesForTimelineChildren,
            )
      ...
      : timelineEventWithDetails.timelineEventDetails
        ...
        : timelineEventWithDetails.timelineEventDetails
          ...
          : timelineEventWithDetails.timelineEventDetails.endMinutes
            ? timelineEventWithDetails.timelineEventDetails.endMinutes
            : timelineEventWithDetails.timelineEventDetails
                .startMinutesForTimelineChildren! + 1;
  ...
  const addEvents = (
    ...
    const createEventNode = (
      ...
      eventHTML.setAttribute(
        "level",
        timelineEvent.timelineEventDetails.level.toString(),
      );
      eventHTML.setAttribute(
        "depth",
        timelineEvent.timelineEventDetails.depth.toString(),
      );
      eventHTML.setAttribute(
        "score",
        timelineEvent.timelineEventDetails.score.toString(),
      );
      eventHTML.setAttribute(
        "step",
        timelineEvent.timelineEventDetails.step.toString(),
      );
    ...
    const setScore = (timelineEvent: ITimelineEventWithDetails) => {
      const durationRatio =
        timelineEvent.timelineEventDetails.durationMinutes /
        parent.timelineEventDetails.durationMinutes;
      const score =
        durationRatio *
        (timelineEvent.timelineEventDetails.childrenByStartMinute.length + 1);
      timelineEvent.timelineEventDetails.score = score;
    };
    const setLevel = (
      timelineEvent: ITimelineEventWithDetails,
      matrix: IMatrix,
      useEqual: boolean = true,
    ) => {
      let level = 0;
      let height = 0;
    ...
    const setNeighborsTo = (
      ...
      timelineEvent.timelineEventDetails.next =
        parent.timelineEventDetails.childrenByStartMinute.length > me + 1
          ? parent.timelineEventDetails.childrenByStartMinute[me + 1]
              .timelineEventDetails.id
          : undefined;
    ...
    const setStep = (timelineEvent: ITimelineEventWithDetails) => {
      timelineEvent.timelineEventDetails.step =
        timelineEvent.step || ++containerStep;
    };

    // push parsed children
    parent.timelineEventDetails.childrenByStartMinute.push(
      ...parsedSortedChildren,
    );
    parent.timelineEventDetails.hasTimelineEvents =
      parent.timelineEventDetails.childrenByStartMinute.some(
        (tl) => tl.type === "timeline",
      );

    // Adjust parent start & end if childrenByStartMinute changed range
    parent.timelineEventDetails.startMinutes = calcStart(parent) ?? 0;
    parent.timelineEventDetails.startMinutesForTimelineChildren =
      calcStartForTimeline(parent);
    parent.timelineEventDetails.endMinutes = calcEnd(parent);
    parent.timelineEventDetails.endMinutesForTimelineChildren =
      calcEndForTimeline(parent);
    parent.timelineEventDetails.durationMinutes =
      parent.timelineEventDetails.endMinutes -
      parent.timelineEventDetails.startMinutes;
    parent.timelineEventDetails.durationMinutesForTimelineChildren =
      parent.timelineEventDetails.endMinutesForTimelineChildren -
      (parent.timelineEventDetails.startMinutesForTimelineChildren ?? 0);

    parent.timelineEventDetails.childrenByStartMinute.forEach(
      (childEvent, i) => {
    ...
    parent.timelineEventDetails.height = Math.max(
      ...Object.keys(parent.timelineEventDetails.timelineLevelMatrix).map(
        (key) => Number(key),
      ),
    );
  };
  ...
  const parseTimelineHTML = (input: HTMLElement): any[] => {
    ...
    const timelineEvents =
      input.querySelectorAll<HTMLElement>(".timelineEvent");
    if (timelineEvents) {
