...
export const timeline: ITimeline = {
  ...
  viewStartMinutes() {
    return (
      this.startMoment.inMinutes -
      this.viewDurationMinutes() * this.options.pivot
    );
  },
  viewEndMinutes() {
  ...
  update() {
    ...
    for (let i = 0; i < this.options.labelCount + 2; i++) {
      const momentInMinutes = (i + 1) * currentTimestampDistanceByLevelMinutes +
        timelineStartMomentExtended +
        currentDifInMinutes -
        currentTimestampDistanceByLevelMinutes;
