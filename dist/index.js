...
var TimelineContainer = (elementIdentifier, settings) => {
  ...
  const init = (elementIdentifier2, settings2) => {
    ...
    if (viewStart2 < timelineStart) timelineStart = viewStart2;
    if (viewEnd2 > timelineEnd) timelineEnd = viewEnd2;
  ...
  const createEventHTML = (parentEvent) => {
    ...
    const isTopLevelContainerChildren = parentEvent.timelineEventDetails.parentId === rootTimeline.timelineEventDetails.id;
    for (const timelineEvent of parentEvent.timelineEventDetails.childrenByStartMinute) {
      ...
      switch (timelineEvent.type) {
        ...
        case "container": {
          ...
          const topLevelContainerOffset = timelineEvent.timelineEventDetails.parentId === rootTimeline.timelineEventDetails.id ? TOP_LEVEL_BAND_OFFSET : 0;
        ...
        case "timeline": {
          ...
          const topLevelTimelineOffset = isTopLevelContainerChildren ? TOP_LEVEL_BAND_OFFSET : 0;
  ...
  const appendContainerHTML = () => {
    ...
    const shouldShowCenterIndicator = options.showCenterMarker || options.showCenterLabel;
  ...
  const appendCenterIndicatorHTML = () => {
    ...
    const parseDateString = (input2) => {
      switch (input2) {
        ...
        default:
          ...
          if (!isNaN(minutes)) {
            return (/* @__PURE__ */ new Date()).getTime() / 6e4 + minutes * 6e4;
          }
          const timestamp = Date.parse(input2);
          if (isNaN(timestamp)) return (/* @__PURE__ */ new Date()).getTime() / 6e4;
  ...
  const addEvents = (parent, ...events) => {
    ...
    const setTopLevelContainerLevel = (timelineEvent, matrix) => {
...
var demoParseToMinutes = (input) => {
  if (input === void 0 || input === null) return void 0;
  if (typeof input === "number") return input;
  if (Array.isArray(input)) return void 0;
  if (input instanceof Date) return input.getTime() / 6e4;
  ...
  return isNaN(ts) ? void 0 : ts / 6e4;
};
var demoClamp = (v, min, max) => Math.max(min, Math.min(max, v));
var demoLerp = (a, b, t) => a + (b - a) * t;
var detectMapType = (timeline) => {
...
var buildLineCoords = (waypoints, centerMinutes) => {
...
var getPhaseAtTime = (phases, centerMinutes) => {
...
var ellipseCoords = (centerLng, centerLat, radiusLng, radiusLat, steps = 72) => {
  const coords = [];
  ...
  map.on("load", () => {
    for (const lt of lineTimelines) {
      const initial = asLineFeature(
        lt.waypoints.length ? [lt.waypoints[0].coordinates, lt.waypoints[0].coordinates] : [
          [0, 0],
          [0, 0]
        ]
      );
    ...
    for (const pt of polygonTimelines) {
      const initial = pt.phases.length ? asPolygonFeature(pt.phases[0]) : asPolygonFeature({
        minutes: 0,
        centerLng: 0,
        centerLat: 0,
        radiusLng: 0.1,
